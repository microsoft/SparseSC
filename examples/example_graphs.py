...
def ind_sc_plots(Y, Y_sc, T0, ind_ci=None):
    ...
    if ind_ci is not None:
        plt.fill_between(range(len(Y_sc)), Y_sc + ind_ci.ci_low, Y_sc + ind_ci.ci_high, facecolor='gray', label="CI")
    plt.axvline(x=T0, linestyle='--')
...
def te_plot(est_ret):
    ...
    effect_vec = np.concatenate((est_ret.pl_res_pre.effect_vec.effect, est_ret.pl_res_post.effect_vec.effect))
    if est_ret.pl_res_pre.effect_vec.ci is not None:
        ci0 = np.concatenate((est_ret.pl_res_pre.effect_vec.ci.ci_low, est_ret.pl_res_post.effect_vec.ci.ci_low))
        ci1 = np.concatenate((est_ret.pl_res_pre.effect_vec.ci.ci_high, est_ret.pl_res_post.effect_vec.ci.ci_high))
    te = plt.figure("te")
    if est_ret.pl_res_pre.effect_vec.ci is not None:
